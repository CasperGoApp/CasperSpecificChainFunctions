...
module.exports = {
  ...
  balance: async (seed, index, erc20Contract) => {
    const mainAccount = await getAccount(seed, index) // get the main account
    // get the balance
    try {
      if (erc20Contract) {
        ...
        return mainAccount.balance // return the balance
      }
    } catch (e) {
      console.log(e.message) // log the error
    }
    ...
    try {
      if (erc20Contract) {
        ...
        return sendTransfer(
          ...
          'casper' + (TESTNET ? '-test' : ''), // network name
  ...
  stake: async (
    ...
    getStakeDeploy(
      ...
      await getAccount(seed, index), // get the account
